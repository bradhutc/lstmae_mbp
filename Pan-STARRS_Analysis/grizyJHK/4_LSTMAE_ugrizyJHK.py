...
class LSTMAutoencoder:
    def __init__(self, input_shape):
        ...
        self.model.add(LSTM(128, activation='elu', input_shape=(input_shape, 9), return_sequences=True))
        ...
        self.model.add(LSTM(9, activation='elu', return_sequences=True))
...
def prepare_data(autoencoder, data, scaler, dataset_type, y_data):
    ...
    reconstructed_magnitudes = autoencoder.reconstruct(data).reshape(-1, 9)

    # Prepare the DataFrame
    original_data_scaled_back = scaler.inverse_transform(data.reshape(-1, 9))
    df = pd.DataFrame(original_data_scaled_back, columns=['psfMag_u','gMeanPSFMag', 'rMeanPSFMag', 'iMeanPSFMag', 'zMeanPSFMag', 'yMeanPSFMag','jmag','hmag','kmag'])
    df[['Gamma', 'Lambda']] = latent_representations
    df[['psfMag_u_prime','gMeanPSFMag_prime', 'rMeanPSFMag_prime', 'iMeanPSFMag_prime', 'zMeanPSFMag_prime', 'yMeanPSFMag_prime','jmag_prime','hmag_prime','kmag_prime']] = scaler.inverse_transform(reconstructed_magnitudes)
    
    # Calculate R-squared score for each magnitude
    r2_scores = {}
    for mag in ['psfMag_u','gMeanPSFMag', 'rMeanPSFMag', 'iMeanPSFMag', 'zMeanPSFMag', 'yMeanPSFMag','jmag','hmag','kmag']:
...
full_data = pd.read_csv(file_path)[['psfMag_u', 'gMeanPSFMag', 'rMeanPSFMag', 'iMeanPSFMag', 'zMeanPSFMag', 'yMeanPSFMag','jmag','hmag','kmag','objID','raMean','decMean','l','b']]
full_data = full_data.dropna()
scaler = MinMaxScaler()
data = full_data[['psfMag_u', 'gMeanPSFMag', 'rMeanPSFMag', 'iMeanPSFMag', 'zMeanPSFMag','yMeanPSFMag','jmag','hmag','kmag','objID','raMean','decMean','l','b']]
data_array = data.to_numpy()
features= ['psfMag_u','gMeanPSFMag', 'rMeanPSFMag', 'iMeanPSFMag', 'zMeanPSFMag','yMeanPSFMag','jmag','hmag','kmag']
X = data[features]
...
# Replace NaN values with 0 or the mean of the scaled data
...
complete_df.to_csv('CSFS_ugrizyJHK.csv', index=False)
